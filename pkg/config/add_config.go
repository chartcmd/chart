...
func AddConfig(key, value string) error {
	...
	case "equities_wl":
		if stocks.IsValidTicker(value) {
			config.EquitiesWatchlist = append(config.EquitiesWatchlist, strings.ToUpper(value))
		} else {
			return fmt.Errorf("invalid equities ticker: %s", value)
		}
	case "crypto_wl":
		if utils.StrSliceContains(c.CryptoList, value, false) {
			config.CryptoWatchlist = append(config.CryptoWatchlist, strings.ToUpper(value))
		} else {
